...
train_target_x = []   
gx = np.load("new_raw_data.npy").astype(np.single)
print (gx.shape)
...
#validation
validation_x = np.fromfile(vdpath + str(5) + '.dat', dtype=np.single)
validation_x = np.reshape(validation_x, dim)
#数据预处理
validation_x = validation_x - np.min(validation_x)
validation_x = validation_x / np.max(validation_x)

validation_y_mid = np.fromfile(vfpath + str(5) + '.dat', dtype=np.single)
validation_y_mid = np.reshape(validation_y_mid, dim)

hot_vali_y_mid = np.reshape(validation_y_mid, 128 * 128 * 128)
hot_vali_y_mid = np.array(hot_vali_y_mid).astype(int)
validation_y = np.zeros((len(hot_vali_y_mid), 2))
#前面为1的为非断层，后面为1的为断层
validation_y[np.arange(len(hot_vali_y_mid)), hot_vali_y_mid] = 1
#(128*128*128*1)后面的1并不是输入的batch_size大小，而是数据的通道,在原始方法中存在数据增强
validation_x = np.reshape(validation_x, ( 1,128, 128, 128, 1))
...
discriminator_feature = Flatten()(class_output)
grl_layer = GradientReversal(1.0)
discriminator_feature = grl_layer(conv_d)
#domain_1 = Dense(128,activation='relu',name = 'dis_1')(discriminator_feature)
#domain_2 = Dense(64,activation='relu',name = 'dis_2')(domain_1)
domain_1 = Dense(128,activation='relu',name = 'dis_1')(discriminator_feature)
discriminator_output = Dense(2,activation="softmax",name="discriminator_output")(domain_1)
...
model.compile(optimizer= Adam(lr = 1e-5),loss={'class_output':cross_entropy_balanced,
                                     'discriminator_output':'categorical_crossentropy'},
              loss_weights = {'class_output':2,'discriminator_output':1 },
              metrics=['accuracy'])
...
#    print (i,'epouch_num')
    if train_mode == 'dann':
        ...
        #p = np.float(i) / epouch_num
        #grl_layer.l = 2. / (1. + np.exp(-10. * p)) - 1
