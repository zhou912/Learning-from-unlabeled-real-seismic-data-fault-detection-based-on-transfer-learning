...
for i in range(200):
    ...
    mini_train_source_x = train_source_x[0:input_num,0:input_num,0:input_num,:]
    mini_train_source_y = train_source_y[0:input_num,0:input_num,0:input_num,:]
...
#目标域数据读入
n1, n2, n3 = input_num, input_num, input_num
...
#gx = raw_data.transpose(0,2,1)
#print (raw_data.shape)
#m1,m2,m3 = 401,101,401
...
#train_target_x_mid = np.concatenate([train_target_x, train_target_x])
#train_target_x = train_target_x_mid[0:200]
#print (train_target_x[100])
#print (train_target_x.shape)
#类别batch生成
...
#构建网络
input_img = Input(shape=(input_num, input_num, input_num,1))
...
callback.set_model(discriminator_model)
...
print ('guanxiedata______________________')
...
#gx = np.load("data.npy").astype(np.single)
#print (gx.shape)
#m1,m2,m3 = 1001,401,321
#os = 12 #overlap width
#c1 = np.round((m1+os)/(n1-os)+0.5)
#c2 = np.round((m2+os)/(n2-os)+0.5)
#c3 = np.round((m3+os)/(n3-os)+0.5)
