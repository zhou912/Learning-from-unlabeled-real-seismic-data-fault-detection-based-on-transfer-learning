...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 31 23:08:31 2019
...
for i in range (1):
    name_epoch += 2000
    json_name = "model+" + str(name_epoch) + ".json"
    json_file = open(json_name, 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
    model_name =str(name_epoch) +  "model3.h5"
...
#    gx,m1,m2,m3 = np.fromfile("transdata_b_yao.dat",dtype=np.single),401,401,101 #yao
    ...
    raw_data = io.loadmat('seismicdata.mat')['seismicdata']
